...
class MongoLib {
    ...
    update(collection, id, data) {
        return this.connect()
            .then(db => {
                return db
                    .collection(collection)
                    .findOneAndUpdate({ _id: ObjectId(id) }, { $set: data }, { upsert: true });
            })
    ...
    delete(collection, id) {
        return this.connect()
            .then(db => {
                ...
                return db.collection(collection).findOneAndDelete({ _id: ObjectId(id) });
            })
            .then(() => id);
    }
    ...
    login(collection, email) {
        return this.connect().then(db => db.collection(collection).findOne({ email: email }));
    }


    getManyComments(collection, id) {
        console.log(id);
        return this.connect().then(db => db.collection(collection).find({ "postid": id }).toArray());
    }

    getManyPosts(collection, id) {
        console.log(id);
        return this.connect().then(db => db.collection(collection).find({ "uid": id }).toArray());
    }
